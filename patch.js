...
const old1 = \  // Dashboard date filters
...
const new1 = \  // Dashboard date filters
...
const old2 = \        if (filterSearchBy === 'Txn ID' && !cb.txnId.includes(filterRrn)) return false;
...
const new2 = \        if (filterSearchBy === 'Txn ID' && !cb.txnId.includes(filterRrn)) return false;
        if (filterSearchBy === 'RRN' && !cb.rrn.includes(filterRrn)) return false;
        if (filterSearchBy === 'TID' && !cb.tid?.includes(filterRrn)) return false;
        if (filterSearchBy === 'MID' && !cb.userId.includes(filterRrn)) return false;
        if (filterSearchBy === 'Case ID' && !cb.caseId?.includes(filterRrn) && !cb.id?.includes(filterRrn)) return false;
...
const old4 = \                  <fieldset style={{ border: '1px solid #d1c4e9', borderRadius: '8px', padding: '24px', marginBottom: '24px', position: 'relative' }}>
...
const new4 = \                  <fieldset style={{ border: '1px solid #d1c4e9', borderRadius: '8px', padding: '24px', marginBottom: '24px', position: 'relative' }}>
                    <legend style={{ padding: '0 8px', color: '#50BDC9', fontWeight: '600', fontSize: '15px', marginLeft: '12px' }}>Search</legend>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
                      {/* Col 1 */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
